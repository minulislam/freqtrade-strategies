...
class NostalgiaForInfinityNext_ChangeToTower_V3(IStrategy):
    INTERFACE_VERSION = 2
...
#                    return True, 'TSI_sell'
                    
            if (current_profit >= 0) & (last_candle[f'pm_{pmaxperiod}_{self.pmaxmultiplier.value}_{self.pmaxlength.value}_{pmaxtype}'] > last_candle[f'mavalue{self.pmaxlength.value}']):
                if (last_candle['crsi'], 80):
                    return True, 'CRSI_dn_sell'
...
#        dataframe['adx'] = ta.ADX(dataframe)
        ...
        dataframe['rsi5'] = ta.RSI(dataframe, 5)
...
#        dataframe['zlema_4_std'] = pta.zlma(dataframe['close'], length = 4)
...
#            item_buy_logic.append(qtpylib.crossed_above(dataframe['zlema_1'], dataframe['zlema_4']))
